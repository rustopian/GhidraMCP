...
class TestListings:
    ...
    def test_list_strings_no_filter(self, bridge_module, httpx_mock):
        httpx_mock.add_response(
            ...
            text='ram:00220dd6: "Sample Program"')
        out = bridge_module.list_strings()
        assert "Sample" in out[0]

    def test_list_strings_with_filter(self, bridge_module, httpx_mock):
        httpx_mock.add_response(
            url=_url("strings?offset=0&limit=10&filter=sample"),
            text='ram:00220dd6: "Sample Program"')
        out = bridge_module.list_strings(limit=10, filter="sample")
        assert len(out) == 1
